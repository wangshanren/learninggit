#include "gps_agent_pkg/dragonplugin.h"
...
// Plugin constructor.
GPSDragonPlugin::GPSDragonPlugin()
{
...
bool GPSDragonPlugin::init(hardware_interface::EffortJointInterface* robot, ros::NodeHandle& n)
{
...
void GPSDragonPlugin::starting(const ros::Time& time)
{
...
void GPSDragonPlugin::stopping(const ros::Time& time)
{
    // Nothing to do here.
}

// This is the main update function called by the realtime thread when the controller is running.
void GPSDragonPlugin::update(const ros::Time& time, const ros::Duration& period)
{
    // Get current time.
    // last_update_time_ = robot_->getTime();
    last_update_time_ = time;
...
// Get current time.
ros::Time GPSDragonPlugin::get_current_time() const
{
    return last_update_time_;
}

// Get current encoder readings (robot-dependent).
void GPSDragonPlugin::get_joint_encoder_readings(Eigen::VectorXd &angles, gps::ActuatorType arm) const
{
...
PLUGINLIB_DECLARE_CLASS(gps_agent_pkg, GPSDragonPlugin,
						gps_control::GPSDragonPlugin,
						controller_interface::ControllerBase)
// PLUGINLIB_EXPORT_CLASS( gps_control::GPSDragonPlugin, controller_interface::ControllerBase )
